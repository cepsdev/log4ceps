...
void monitor_thread_func()
{
	...
	for (; !exit_request;)
	{
		...
		if (get_value<0>(m)) 
			log4kmw_loggers::log_event(log4kmw_events::MonitorAn(), log4kmw_loggers::logger_status_all);
		else
			log4kmw_loggers::log_event(log4kmw_events::MonitorAus(), log4kmw_loggers::logger_status_all);
	}
}
